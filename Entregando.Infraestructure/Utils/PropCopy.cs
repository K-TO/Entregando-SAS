...
namespace Entregando.Infraestructure.Utils
{
    /// <summary>
    /// Funcionalidad que permite clonar objetos.
    /// </summary>
    public static class PropCopy
    {
        /// <summary>
    ...
    internal static class PropertyCopier<TSource, TTarget>
    {
        ...
        internal static void Copy(TSource source, TTarget target)
        {
            ...
            //Si se recibió un JObject
            if (source.GetType().FullName.Equals("Newtonsoft.Json.Linq.JObject"))
            {
                List<JProperty> sourceJProperties = CustomJObjectCreator(source, target);

                for (int i = 0; i < sourceJProperties.Count; i++)
                {
                    try
                    {
                        if (Nullable.GetUnderlyingType(targetProperties[i].PropertyType) != null)
                            targetProperties[i].SetValue(target, Convert.ChangeType(((JValue)(sourceJProperties[i].Value)).Value, targetProperties[i].PropertyType.GenericTypeArguments[0]), null);
                        else
                            targetProperties[i].SetValue(target, Convert.ChangeType(((JValue)(sourceJProperties[i].Value)).Value, targetProperties[i].PropertyType), null);
                    }
                    catch (InvalidCastException)
                    {
                        //Si los tipos son incompatibles no se hace el casteo
                    }
                }
            }
            else
        ...
        internal static List<JProperty> CustomJObjectCreator(TSource source, TTarget target)
        {
            JObject obj = (JObject)(Object)source;
            var properties = new List<JProperty>();

            var bindings = new List<MemberBinding>();
            targetProperties.Clear();
            foreach (JProperty sourceProperty in obj.Properties())
            {
        ...
        private static Func<TSource, TTarget> BuildCreator()
        {
            ...
            foreach (PropertyInfo sourceProperty in typeof(TSource).GetProperties(BindingFlags.Public | BindingFlags.Instance))
            {
                ...
                PropertyInfo targetProperty = typeof(TTarget).GetProperty(sourceProperty.Name);
                //No todas las propiedades deben existir
                //if (targetProperty == null)
                //{
